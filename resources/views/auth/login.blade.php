...
<div class="d-flex flex-column align-items-center justify-content-center" style="min-height:70vh;">
    <div class="card shadow-lg border-0 rounded-4" style="max-width: 400px; background: rgba(255,255,255,0.95);">
        <div class="card-body p-4">
            <h2 class="fw-bold text-center mb-4 text-primary">@lang('messages.login')</h2>
    <!-- Session Status -->
    ...
    <form method="POST" action="{{ route('login') }}">
        @csrf
            ...
            @endif
                ...
                <button type="submit" class="btn btn-primary btn-lg w-100 rounded-pill">{{ __('messages.log_in') }}</button>
            </form>
        </div>
    </div>
</div>
